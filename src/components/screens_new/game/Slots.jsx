...
const BonusPage = () => {
    ...
    return (
        <div className="bg-gradient-to-b from-red-900 via-red-800 to-black min-h-screen flex flex-col items-center text-white p-4 overflow-hidden">
            <div
                className="w-full max-w-3xl flex-col flex gap-4 mx-auto bg-black bg-opacity-20 rounded-xl shadow-lg overflow-hidden p-4 z-10">
                {backgroundEmojis.map(emoji => (
                    <div key={emoji.id} className="absolute text-2xl" style={{
                        left: `${emoji.x}%`, top: `${emoji.y}%`, fontSize: `${emoji.size}px`,
                        transition: 'all 0.5s linear',
                    }}>{emoji.emoji}</div>
                ))}
                <h2 className="text-5xl font-bold text-center text-yellow-400 mb-2">EMOJI PARTY</h2>
                ...
                <div className="text-center mb-2">
                    <p className="text-4xl font-bold">⭐️ {score}</p>
                </div>
                <div className="bg-gradient-to-r from-indigo-700 to-purple-700 rounded-lg p-3 relative bg-opacity-70">
                    <div className="flex justify-around mb-3 relative overflow-hidden h-40">
                        {[0, 1, 2].map(reelIndex => (
                            <div key={reelIndex} className="w-1/3 overflow-hidden h-full relative">
                                <div
                                    className="absolute inset-0 bg-gradient-to-b from-black via-transparent to-black pointer-events-none z-10"></div>
                                <div
                ...
                <button
                    ...
                    className="w-full bg-gradient-to-r from-blue-500 to-blue-700 text-white py-2 px-4 rounded-lg mb-2 transform transition duration-200 hover:scale-105"
                >
